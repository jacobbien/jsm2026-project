...
knit: litr::render
params:
  package_name: "jsm2026" # <-- change this to your package name
  package_parent_dir: "." # <-- relative to this file's location
---

<!-- This Rmd file contains all the code needed to define an R package.  Press "Knit" in RStudio or more generally run `rmarkdown::render("create-jsm2026.Rmd")` to generate the R package.  Remember that when you want to modify anything about the R package, you should modify this document rather than the package that is outputted.
-->
...
#' 
#' @docType package
#' @seealso [`jsm2026::get_talks`], [`jsm2026::get_coauthors`], [`jsm2026::get_out_citations`], [`jsm2026::get_in_citations`], [`jsm2026::export_calendar_to_ics`], [`jsm2026::export_calendar_to_csv`]
```
...
Let's also add a character vector of words appearing in the titles and abstracts:
...
```{r}
#' Words used in this year's JSM titles and abstracts
#' @format A character vector of `r length(words)` words
#' @source From this year's JSM program.
"words"
```
...
### Authorship, coauthorship, and citation information
...
1. Metadata from all arxiv papers in stat.XX from January 1, 2015 to July 15, 2026 (using the [arxiv bulk metadata access OAI protocol](https://arxiv.org/help/oa/index)).  There were 129,958 such papers.

2. Semantic Scholar (S2)'s bulk data sets (on papers, authors, and citations), in which we started with about 200M papers and filtered down to about 460K papers (written by about 638K authors) by restricting attention to papers in statistics journals or having the arxiv stat.XX tag.

3. The JSM 2026 program itself, along with the list of authors that was scraped from JSM 2022-2025.  There are `r length(authors)` names in total.
...
```{r}
#' JSM author names
#' @format A character vector of `r length(authors)` names from past and present JSMs
#' @source This data was from the JSM 2022-2026 programs.
"authors"
```
...
```{r}
authors_thisyear <- jsm$authors_thisyear
```

There are `r length(authors_thisyear)` authors in this year's JSM.  We will want to have precomputed recommendations available for everyone in `authors`.  This combines everyone who has appeared on the JSM program 2022-2026.  However, we want these recommendations to be about only those people who are actually giving talks in 2026.  (E.g., it's not useful to see the name of a coauthor of mine if they aren't presenting at JSM this year!)

```{r}
#' JSM author names for this year
#' @format A character vector of `r length(authors_thisyear)` author names
...
```{r}
usethis::use_data(authors_thisyear)
```
...
desc::desc_set(
  "URL",
  "https://github.com/jacobbien/jsm2026-project/tree/main/jsm2026"
  )
```
...
```{r}
add_hex_sticker("../source-files/jsm2026-hex.png")
```
